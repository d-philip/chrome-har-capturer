...
class Live {
    ...
    async _loadPage(client) {
        ...
        const networkPreset = await this._options.emulateNetwork;
        if (networkPreset) {
            if (networkPreset in conditions) {
                await Network.emulateNetworkConditions(conditions[networkPreset]);
                console.log('Network conditions emulated');
            }
            else {
                throw new Error(`No network condition preset named "${networkPreset}"`);
            }
        }
